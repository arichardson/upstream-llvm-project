// RUN: llvm-mc --triple=riscv64 --mattr=+xllvmrvyipm --riscv-no-aliases --show-encoding --show-inst < %s \
...
// RUN: llvm-mc --filetype=obj --triple=riscv64 --mattr=+xllvmrvyipm --riscv-add-build-attributes < %s \
// RUN:   | llvm-objdump -M no-aliases -d -r --no-print-imm-hex - | FileCheck --check-prefixes=CHECK,CHECK-OBJ %s
