...
# RUN: llvm-mc --triple=riscv32 --mattr=+experimental-zyhybrid --riscv-no-aliases --show-encoding < %s \
# RUN:   | FileCheck --check-prefixes=CHECK-ASM,CHECK %s
# RUN: not llvm-mc --triple=riscv32 --mattr=+experimental-y --riscv-no-aliases --show-encoding < %s 2>&1 \
# RUN:   | FileCheck --check-prefixes=ERR-RVY --implicit-check-not=error: %s
# RUN: llvm-mc --filetype=obj --triple=riscv32 --mattr=+experimental-zyhybrid -o %t.rv32i.o < %s
# RUN: llvm-objdump --mattr=+experimental-zyhybrid -M no-aliases -d --no-print-imm-hex %t.rv32i.o \
...
# RUN: llvm-mc --triple=riscv64 --mattr=+experimental-zyhybrid --riscv-no-aliases --show-encoding --defsym=RV64=1 < %s \
# RUN:   | FileCheck --check-prefixes=CHECK-ASM,CHECK-ASM-64,CHECK,CHECK-64 %s
# RUN: not llvm-mc --triple=riscv64 --mattr=+experimental-y --riscv-no-aliases --show-encoding --defsym=RV64=1 < %s 2>&1 \
# RUN:   | FileCheck --check-prefixes=ERR-RVY,ERR-RVY-64 --implicit-check-not=error: %s
# RUN: llvm-mc --filetype=obj --triple=riscv64 --mattr=+experimental-zyhybrid --riscv-no-aliases --show-encoding --defsym=RV64=1 -o %t.rv64i.o < %s
# RUN: llvm-objdump --mattr=+experimental-zyhybrid -M no-aliases -d --no-print-imm-hex %t.rv64i.o \
